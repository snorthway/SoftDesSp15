...
class GridWorld():
    ...
    def _add_lava(self, mouse_pos):
        ...
        if self._is_occupied(lava_coord):
            if self.actors[lava_coord].unremovable is False:
...
class Paul(Actor):
    ...
    def get_open_adj_coords(self, coords):
        ...
        for i, coord in enumerate(adj_coords):  # note to self, this is nice syntax
            costs[i] += self.world.get_terrain_cost(coord)

        # "Paul gets hops"
        for c in adj_coords:
            if self.world._is_in_grid(c) and c not in self.closed_list:
                jump_coord = tuple([x*2 for x in c])
                if self.world._is_occupied(c) and not self.world._is_occupied(jump_coord):
                    adj_coords.append(jump_coord)
                    costs.append(8)
